...
public class KaaController extends AbstractController {
    ...
    public ResponseEntity<KaaResponse<List<KaaEndPointDataDTO>>> getDataByDate(@RequestParam(value = FROM, required = false) final String fromDate,
                                                                               ...
                                                                               @RequestParam(value = PERIOD_SAMPLE, required = false) final String periodSample) throws Exception {

        return ResponseEntity.status(HttpStatus.OK)
                .body(new KaaResponse<>(HttpStatus.OK.value(), HttpStatus.OK.getReasonPhrase(),
                        kaaService.getKaaEndPointsData(fromDate, toDate, includeTime, sort, periodSample)));
    }

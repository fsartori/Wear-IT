...
public abstract class AbstractController {
    ...
    protected void validateOutputFormat(final String outputFormat) throws RequestException {
        if (!Arrays.asList("csv", "xml", "json").contains(outputFormat))
            throw new RequestException(HttpStatus.BAD_REQUEST, "Invalid output format");
    ...
    protected void validateParams(final KaaEndpointDataRequest data) throws RequestException {
        if (Objects.isNull(data.getEndpointId()) && Objects.isNull(data.getValues()))
            throw new RequestException(HttpStatus.BAD_REQUEST, "No data to send");
    }
    ...
    protected void validateParams(final List<String> endpointList, final String dataName) throws RequestException {
        if (StringUtils.isEmpty(dataName) && CollectionUtils.isEmpty(endpointList))
            throw new RequestException(HttpStatus.BAD_REQUEST, "Invalid params, endpointId and dataName must be not empty");
    }
    ...
    protected ResponseEntity<BaseResponse> createResponse(final boolean sent) {
        return sent ? ResponseEntity.status(HttpStatus.ACCEPTED).body(BaseResponse.builder().status(HttpStatus.ACCEPTED).message("the data has been sent succesfully").build()) :
                ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(BaseResponse.builder().status(HttpStatus.INTERNAL_SERVER_ERROR).message("unexpected error while sending data").build());
    }
    ...
    private ResponseEntity<?> createXMLResponse(final List<KaaEndPointDataDTO> kaaEndPointDataDTOS) {
        return ResponseEntity
                ...
                .body(new KaaResponse<>(HttpStatus.OK, HttpStatus.OK.getReasonPhrase(), kaaEndPointDataDTOS));
    }
